            ...
            <ValidationObserver v-slot="{ handleSubmit }">
                ...
                <section class="modal-card-body">
                    ...
                    <div class="field" v-if="form.type !== 'ChangeCurrency'">
                        ...
                        <div class="control">
                            <ValidationProvider name="Amount" rules="required|numeric" v-slot="{ errors }">
                                <currency-input v-model="form.adjustmentAmount" class="input" :allow-negative="false" :value-range="{ min: 0 }" :distraction-free="false" :currency="{ prefix: '', suffix: '' }" :value-as-integer="true" :precision="0" />
                                <p class="has-text-danger">{{ errors[0] }}</p>
                            </ValidationProvider>
                        </div>
                    </div>
                    ...
                    <div v-if="form.type === 'ChangeCurrency'" class="field">
                        <label class="label">Before / After Change Currency 

                          <span class="has-text-info">( {{ `${currencyName(oldCurrency)} to ${currencyName(form.currencyId)}`   }} )</span>

                        </label>
                        <div class="control columns" style="margin: 5px 0 5px">
                            <input :value="beforeAdjustment | currency" type="text" class="input" disabled>
                            <input :value="form.adjustmentAmount ? form.adjustmentAmount : 0 | currency" type="text" class="input" disabled>
                        </div>
                    </div>
                                        <div v-else class="field">
                        <label class="label">Before / After Adjustment
